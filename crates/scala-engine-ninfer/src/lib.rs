...
fn apply_ninfer_runtime_contract(
    definitions: &mut [scala_core::SettingDefinition],
    help: &str,
    capabilities: NinferRuntimeCapabilities,
) {
    for definition in definitions.iter_mut() {
        let id = definition.id.as_str();
        let unsupported = |definition: &mut scala_core::SettingDefinition, reason: &str| {
        ...
        match settings::execution_path_for_setting(id) {
            ...
            | settings::SettingExecutionPath::ScalaRequestDefault => {}
        }
    }
    if let Some(definition) = definitions
        .iter_mut()
        .find(|definition| definition.id.as_str() == "ninfer.kv_dtype")
        && let scala_core::SettingKind::Choice { choices } = &mut definition.kind
    {
...
fn ninfer_startup_requirements(
    ...
    let seed = match settings.value("ninfer.seed") {
        Some(SettingValue::UnsignedIntegerOrChoice(
            scala_core::UnsignedIntegerOrChoiceValue::UnsignedInteger(value),
        )) => Some(*value),
        Some(SettingValue::UnsignedIntegerOrChoice(
        ...
        if let Some(config) = config {
                ...
                .find(|name| conflicts_with_managed_environment(name))
            {
                configuration_error = Some(format!(
                    "environment variable `{name}` conflicts with Scala's exact NInfer CUDA device isolation"
                ));
            }
            ...
            {
                configuration_error = Some(format!(
                    "native argument `{argument}` conflicts with the Scala-managed NInfer contract"
                ));
            }
            if configuration_error.is_none()
...
impl EngineAdapter for NinferAdapter {
    ...
    fn setting_definitions(&self) -> Vec<scala_core::SettingDefinition> {
    ...
    fn model_setting_definitions(
        &self,
        model: &ModelArtifact,
    ) -> Result<Vec<scala_core::SettingDefinition>, EngineError> {
    ...
    async fn runtime_settings_schema(
        ...
        _host: &HostCapabilities,
        settings: Option<&ResolvedSettings>,
    ) -> Result<scala_core::SettingsSchema, EngineError> {
    ...
    async fn startup_observation(
        ...
        _stderr_tail: &[String],
    ) -> Result<scala_engine::StartupObservation, EngineError> {
        let endpoint = process.endpoint.as_deref().ok_or_else(|| {
            EngineError::Operation("NInfer process has no backend endpoint".to_owned())
        })?;
        let observed = self
        ...
        Ok(scala_engine::StartupObservation::Ready(BTreeMap::from([
            (
...
async fn bind_ninfer_context_cost_presets(
    ...
    let id = scala_core::SettingId::new(SETTING_ID).expect("static NInfer setting ID");
...
mod tests {
    ...
    fn managed_source_recipe_generations_share_the_ninfer_update_line() {
        let identity = |variant: &str| scala_core::RuntimeIdentity {
            engine_id: ENGINE_ID.to_owned(),
