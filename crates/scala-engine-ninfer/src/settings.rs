...
use scala_engine::{EngineError, common_setting_definitions_for};
...
pub(crate) fn definitions() -> Vec<SettingDefinition> {
    ...
    definitions.extend([
        ...
        unsigned(
            ...
            "Private NInfer request-body limit in MiB; Scala's public limit remains authoritative",
            1,
            Some(u64::MAX >> 20),
        ),
        unsigned(
...
fn dflash2_target(model: &ModelArtifact) -> bool {
    scala_core::inspect_ninfer_container(&model.path).is_ok_and(|metadata| metadata.dflash2)
}
...
pub(crate) fn execution_path_for_setting(id: &str) -> SettingExecutionPath {
    use SettingExecutionPath::{
        LaunchOption, ScalaRequestDefault, Unsupported, VirtualLaunchControl,
    };
    ...
    match id {
        ...
        | "ninfer.reasoning"
        | "ninfer.stop_strings"
        | "ninfer.system_prompt" => ScalaRequestDefault,
...
pub(crate) fn translate(
    ...
    for id in settings.configured.keys() {
        let option = match execution_path_for_setting(id.as_str()) {
            ...
            SettingExecutionPath::ScalaRequestDefault
...
mod tests {
    ...
    fn model(model_id: &str) -> ModelArtifact {
        ModelArtifact {
            ...
            format: scala_core::ArtifactFormat::Ninfer,
            size_bytes: 1,
            created: 1,
            hash: None,
            architecture: None,
