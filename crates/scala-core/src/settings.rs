...
impl std::fmt::Display for SettingDefaultSource {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        formatter.write_str(match self {
            Self::Scala => "server execution policy",
...
/// Scala-owned scalar may also be materialized into the launch contract.
#[derive(Debug, Clone, Eq, PartialEq, Serialize, Deserialize)]
...
impl SettingsSchema {
    ...
    pub fn materialize_runtime_configuration(
            ...
            .filter(|definition| definition.supported)
        {
            ...
            if runtime_default.source != SettingDefaultSource::Scala {
                continue;
            }
