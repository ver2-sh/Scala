...
pub const MODEL_LIBRARY_RECEIPT_FILENAME: &str = ".scala-library.json";
...
/// Returns true for Scala-internal directory names that must never be
...
fn is_scala_internal_entry(name: &std::ffi::OsStr) -> bool {
    if name == std::ffi::OsStr::new(".scala-staging") {
        return true;
    }
    name.to_string_lossy()
        .strip_prefix(".scala-install-")
        .is_some()
}
    ...
    pub fn discover(search_paths: &[PathBuf]) -> Self {
        ...
        for root in search_paths {
            ...
            for entry in WalkDir::new(root)
                .follow_links(false)
                .into_iter()
                .filter_entry(|entry| !is_scala_internal_entry(entry.file_name()))
            {
...
fn model_id(
    ...
    let digest = Sha256::digest(format!(
        // Immutable identity domain: changing a product name must not change ModelIds.
...
mod tests {
    ...
    fn q27_primary_keeps_its_id_and_owns_the_tokenizer_companion() {
        let root =
            std::env::temp_dir().join(format!("scala-q27-companion-{}", uuid::Uuid::new_v4()));
    ...
    fn q27_upstream_model_and_tokenizer_names_pair_when_tokenizer_is_unique() {
        let root =
            std::env::temp_dir().join(format!("scala-q27-upstream-{}", uuid::Uuid::new_v4()));
        std::fs::create_dir(&root).expect("model fixture directory");
