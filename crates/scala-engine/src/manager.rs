...
struct ManagedBackend {
    ...
    runtime_id: Option<RuntimeId>,
    accelerator_binding: Option<scala_core::AcceleratorBinding>,
...
fn backend_parallelism(backend: &ManagedBackend) -> Option<BackendParallelism> {
    ...
    match running.settings.runtime_value("parallel_requests") {
        Some(scala_core::SettingValue::UnsignedInteger(value)) => {
            Some(BackendParallelism::Exact(*value))
        }
        Some(scala_core::SettingValue::UnsignedIntegerOrChoice(
        ...
        Some(scala_core::SettingValue::Choice(value)) if value.eq_ignore_ascii_case("auto") => {
...
fn prepare_generation_patch(
    ...
    if patch.min_p.is_none()
        && let Some(scala_core::SettingValue::Float(value)) = settings.runtime_value("min_p")
    {
        patch.min_p = Some(*value);
    }
    if patch.seed.is_none()
        && let Some(scala_core::SettingValue::UnsignedIntegerOrChoice(
            scala_core::UnsignedIntegerOrChoiceValue::UnsignedInteger(value),
    ...
    if patch.repeat_penalty.is_none()
        && let Some(scala_core::SettingValue::Float(value)) =
    ...
    if patch.stop.is_none()
        && let Some(scala_core::SettingValue::StringList(value)) =
...
async fn prepare_inference_request(
    ...
    }) && let Some(scala_core::SettingValue::String(prompt)) =
    ...
    if request.output_format.is_none()
        && let Some(scala_core::SettingValue::Json(schema)) =
    ...
    if !matches!(
        ...
        Some(scala_core::SettingValue::Choice(policy)) if policy == "truncate_middle"
    ) {
        return Ok(());
    }
...
fn reconcile_effective_settings_from_runtime(provenance: &mut RuntimeProvenance) {
    ...
    for (raw_id, value) in resolved {
        let Ok(id) = scala_core::SettingId::new(raw_id.clone()) else {
            continue;
        };
        let display = value
            .as_str()
        ...
        match provenance.settings.effective.get_mut(&id) {
            ...
            None => {
                provenance.settings.effective.insert(
                    id,
                    scala_core::EffectiveSetting {
                        value: display,
                        source: scala_core::SettingSource::RuntimeDefault,
