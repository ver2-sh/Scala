...
impl RuntimeManager {
    ...
    async fn saved_benchmark_configuration(
        ...
        let resolved = settings
            .resolve(
                ...
                &profile.overrides,
                &scala_core::SettingsPatch::default(),
        ...
        let selections = match tokio::fs::read(&self.core.paths.runtime_selections_file).await {
            Ok(bytes) if bytes.len() <= 1024 * 1024 => {
                serde_json::from_slice::<scala_core::RuntimeSelections>(&bytes)
                    .map_err(|e| e.to_string())?
            }
    ...
    async fn start_benchmark(
        ...
        *self.benchmark.active.lock().await = Some(Active {
        ...
        let run = Run {
            ...
            source_revision: option_env!("SCALA_SOURCE_REVISION").map(str::to_owned),
            provenance: None,
            ...
            missing: vec![
                ...
                "Source revision unavailable unless SCALA_SOURCE_REVISION was set at build time".into(),
        ...
        tokio::time::timeout(Duration::from_secs(plan.preparation_seconds).saturating_sub(started.elapsed()),async {
            ...
            if run.configuration_key.is_none() {run.missing.push("Current configuration equivalence unverified: missing identity, file observation, accelerator binding or fallback next-load runtime selection".into());}
            {
                ...
                if let Some(running)=state.backends.get(&run.profile.id).and_then(|b|b.running.as_ref()) {
                    let settings=self.settings.read().await.map_err(|e|e.to_string())?;
                    let saved=settings.resolve(&run.profile.id,run.profile.engine_id.as_str(),&run.profile.overrides,&scala_core::SettingsPatch::default(),&self.core.paths.data_dir).map_err(|e|e.to_string())?;
                    run.environment["requested_saved_settings"]=json!(saved.configured);
                    let served_requested=running.settings.configured.iter().filter(|(_,setting)|!matches!(setting.source,scala_core::SettingSource::RuntimeDefault)).map(|(id,value)|(id.clone(),value.clone())).collect::<BTreeMap<_,_>>();
    ...
    async fn request(
        ...
        let mut max = match r.settings.runtime_value("max_output_tokens") {
            Some(scala_core::SettingValue::UnsignedInteger(v)) => {
...
fn configuration_key(
    p: &scala_core::RuntimeProvenance,
    saved: &Value,
    environment: &Value,
) -> String {
...
async fn served_files(p: &scala_core::RuntimeProvenance) -> Value {
    let mut files = vec![
...
fn verified_key(p: &scala_core::RuntimeProvenance, saved: &Value, env: &Value) -> Option<String> {
    ...
    if missing(saved)
        ...
        || matches!(
            p.selection_source,
            scala_core::RuntimeSelectionSource::Fallback
        )
...
async fn setting_file_observations(settings: &scala_core::ResolvedSettings) -> Value {
    let mut files = Vec::new();
    for (id, value) in &settings.configured {
        if let scala_core::SettingValue::Path(path) = &value.value {
            files.push(
