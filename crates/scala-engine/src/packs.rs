...
impl RuntimePackManager {
    ...
    pub async fn settings_schema_for_model(
        &self,
        model: &scala_core::ModelArtifact,
        explicit_runtime: Option<&scala_core::RuntimeId>,
    ) -> Result<(scala_core::RuntimeSelection, scala_core::SettingsSchema), RuntimePackError> {
    ...
    pub async fn selected_runtime_settings_schemas(
        &self,
        settings_state: &scala_core::SettingsState,
        structured_path_base: &Path,
    ) -> Result<(BTreeMap<String, scala_core::SettingsSchema>, Vec<String>), RuntimePackError> {
        if !self
            .host_initialized
            .load(std::sync::atomic::Ordering::Acquire)
        {
        ...
        let host = self.host_capabilities().await;
        let list = self.list().await?;
        let mut schemas = BTreeMap::<String, scala_core::SettingsSchema>::new();
        ...
        for adapter in self.registry.adapters() {
            ...
            let mut schema = scala_core::SettingsSchema {
    ...
    pub fn model_settings_schema_for_engine(
        ...
        model: &scala_core::ModelArtifact,
        engine_id: &str,
    ) -> Result<scala_core::SettingsSchema, RuntimePackError> {
        let adapter = self.registry.get(engine_id).ok_or_else(|| {
            RuntimePackError::Selection(format!("bound engine `{engine_id}` is not registered"))
        })?;
    ...
    async fn search_internal(
        ...
        query: &str,
        force_refresh: bool,
        model: Option<(&ModelArtifact, Option<&scala_core::ResolvedSettings>)>,
    ) -> Result<RuntimeSearchSnapshot, RuntimePackError> {
        let host = self.host.read().await.clone();
    ...
    pub async fn settings_schema_from_local_inspection(
        ...
        settings: Option<&scala_core::ResolvedSettings>,
        inspection: &RuntimeLocalInspection,
    ) -> Result<(RuntimeSelection, scala_core::SettingsSchema), RuntimePackError> {
        let selection =
...
fn ensure_catalog_provenance_matches(
    ...
    let acquisition_matches = match &available.acquisition {
        ...
        scala_core::RuntimeAcquisitionPlan::SourceBuild(plan) => {
...
fn source_build_provenance_matches_plan(
    provenance: &scala_core::RuntimeSourceBuildProvenance,
    plan: &scala_core::RuntimeSourceBuildPlan,
) -> bool {
...
mod tests {
    ...
    impl EngineAdapter for BoundSchemaAdapter {
        ...
        async fn settings_schema(
            ...
            _host: &scala_core::HostCapabilities,
            _settings: Option<&scala_core::ResolvedSettings>,
            ...
            Ok(SettingsSchema {
                ...
                definitions: vec![SettingDefinition {
                    ...
                    category: scala_core::SettingCategory::Advanced,
                    supported: true,
