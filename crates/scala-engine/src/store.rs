...
pub enum RuntimeStoreError {
    ...
    #[error("external runtime `{0}` is unmanaged and cannot be removed by Scala")]
...
mod tests {
    ...
    async fn stage_runtime(
        store: &RuntimeStore,
        manifest: &scala_core::RuntimeManifest,
        entrypoint: &[u8],
    ) -> RuntimeStaging {
    ...
    async fn failed_probe_manifest_cannot_be_activated() {
        ...
        let manifest = scala_core::RuntimeManifest {
    ...
    async fn running_runtime_lease_blocks_exclusive_removal_lease() {
        ...
        let store = RuntimeStore::new(&paths);
        let runtime_id = scala_core::RuntimeId::new("leased-runtime").expect("runtime ID");
        let _running = store
    ...
    async fn installation_lease_is_exclusive_per_exact_runtime() {
        ...
        let first_id = scala_core::RuntimeId::new("first-runtime").expect("runtime ID");
        let second_id = scala_core::RuntimeId::new("second-runtime").expect("runtime ID");
