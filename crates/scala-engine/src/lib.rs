...
fn common_setting_definition_library(engine_id: &str) -> Vec<SettingDefinition> {
    vec![
        SettingDefinition {
            ...
            kind: scala_core::SettingKind::UnsignedInteger {
                minimum: Some(1),
                maximum: None,
            },
            scope: scala_core::SettingScope::Runtime {
                engine_id: engine_id.to_owned(),
            },
            category: scala_core::SettingCategory::General,
            supported: true,
        ...
        SettingDefinition {
            ...
            description: "Configured generation temperature; request values may override it"
                .to_owned(),
            kind: scala_core::SettingKind::Float {
                minimum: Some(0.0),
                maximum: None,
            },
            scope: scala_core::SettingScope::Runtime {
            ...
            category: scala_core::SettingCategory::Generation,
            supported: true,
            unsupported_reason: None,
            unit: None,
            default_preview: None,
        ...
        SettingDefinition {
            ...
            kind: scala_core::SettingKind::Choice {
            ...
            category: scala_core::SettingCategory::Reasoning,
            supported: true,
            unsupported_reason: None,
            unit: None,
            default_preview: None,
        ...
        common_definition(
            ...
            scala_core::SettingKind::UnsignedIntegerOrChoice {
                minimum: Some(0),
        ...
        common_definition(
            ...
            scala_core::SettingKind::UnsignedInteger {
                minimum: Some(1),
                maximum: Some(u64::from(u32::MAX)),
            },
            scala_core::SettingCategory::Generation,
        ...
        common_definition(
            ...
            scala_core::SettingKind::StringList,
        ...
        common_definition(
            ...
            scala_core::SettingKind::Choice {
                choices: ["on", "off", "auto"]
                    .into_iter()
                    .map(str::to_owned)
                    .collect(),
            },
            scala_core::SettingCategory::Reasoning,
            engine_id,
        ),
        common_definition(
            "reasoning_budget",
            ...
            scala_core::SettingKind::Integer {
                minimum: Some(-1),
        ...
        common_definition(
            ...
            scala_core::SettingKind::JsonObject,
            scala_core::SettingCategory::Generation,
            engine_id,
        ),
        common_definition(
...
fn common_definition(
    ...
    label: &str,
    description: &str,
    kind: scala_core::SettingKind,
    category: scala_core::SettingCategory,
    engine_id: &str,
) -> SettingDefinition {
    SettingDefinition {
        ...
        default_preview: match id {
            "stop_strings" | "system_prompt" | "reasoning_budget_message" => Some(
                SettingDefaultPreview::new("None", SettingDefaultSource::Scala),
            ),
...
pub async fn record_local_file_setting_identity(
    ...
    let Some(scala_core::SettingValue::Path(path)) = patch.0.get(&path_id) else {
        return Ok(());
    };
    let observed =
        scala_core::bounded_setting_file_sha256(&path_id, path, path_base, maximum_bytes).await?;
    let sha256_id = SettingId::new(sha256_setting)?;
    if let Some(configured) = patch.0.get(&sha256_id) {
        match configured {
            scala_core::SettingValue::String(configured)
    ...
    patch.insert(sha256_id, scala_core::SettingValue::String(observed));
    Ok(())
}
    ...
    pub fn runtime_identity(&self) -> ModelRuntimeIdentity {
        ModelRuntimeIdentity {
            ...
            auxiliary: self
                .auxiliary
                .iter()
                .map(|artifact| scala_core::AuxiliaryRuntimeIdentity {
            ...
            norted_package: self
                ...
                .norted_package
                .clone()
                .map(|binding| scala_core::NortedPackageRuntimeIdentity { binding }),
        }
    }
}
...
pub struct NortedPackageSummary {
    pub kind: scala_core::NortedPackageKind,
...
pub struct InferenceRequest {
    pub model_profile_id: scala_core::ModelProfileId,
...
pub trait EngineAdapter: Send + Sync {
    ...
    /// carry through Scala's public and private translations end to end.
    fn serving_features(
        &self,
        _runtime: &InstalledRuntime,
    ...
    fn runtime_model_accelerator_binding(
        ...
        _host: &HostCapabilities,
        _settings: Option<&ResolvedSettings>,
    ) -> Option<scala_core::AcceleratorBinding> {
        None
    }
...
mod tests {
    ...
    async fn blocking_hash_matches_reference_for_empty_and_small_files() {
        ...
        for (name, bytes) in [("empty", &b""[..]), ("small", &b"scala hash fixture"[..])] {
    ...
    async fn manual_blocking_hash_benchmark() {
        let path = std::env::var_os("SCALA_HASH_BENCH_FILE")
            .map(PathBuf::from)
            .expect("set SCALA_HASH_BENCH_FILE");
    ...
    impl EngineAdapter for ArchitectureAdapter {
        ...
        async fn probe_runtime(
            &self,
            _runtime: &scala_core::InstalledRuntime,
        ) -> Result<scala_core::RuntimeProbeObservation, EngineError> {
            unreachable!()
        }
    ...
    async fn package_preparation_hashes_primary_in_place_and_rejects_tampering() {
        ...
        assert_eq!(summary.kind, scala_core::NortedPackageKind::Gguf);
    ...
    fn backend_status_serializes_an_optional_load_progress_state() {
        let with_progress = BackendStatus {
            ...
            model_profile_id: scala_core::ModelProfileId::new("qwen-quality").expect("profile ID"),
            model_id: ModelId("qwen3.8-27b".to_owned()),
            role: scala_core::ModelRole::Primary,
