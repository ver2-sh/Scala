...
use norted_core::{AppPaths, ModelLibraryReceipt, model_library_receipt_path, prune::PrunePlan};
...
pub fn plan_model_prune(
    ...
    if plan.mode == "all" {
        ...
        // directory (e.g. /mnt/models). Only Norted-owned managed acquisitions
        // beneath it are reclaimable: a directory is treated as a complete
        // managed acquisition only when it lives at a known Norted layout and
        // carries a valid `.norted-library.json` receipt. Unrelated content
...
/// Schedule deletion of Norted-owned managed acquisitions beneath the
...
/// root carries a valid `.norted-library.json` receipt. The download root
...
/// Confirm that `path` is a genuine Norted-managed acquisition by validating
/// its `.norted-library.json` receipt against the existing model-library
/// ownership contract. Invalid, missing, or unparseable receipts fail safe:
/// the directory is treated as not Norted-owned and preserved rather than
