    ...
    match app.screen {
        ...
        Screen::Link => frame.render_widget(
            Paragraph::new(app.link_detail())
                ...
                .block(Block::bordered().title("Scala Link")),
            area,
        ),
...
fn discovered_state(
    app: &App,
    repository: &scala_model_library::CatalogRepository,
    artifact: &scala_model_library::CatalogFile,
) -> String {
    if let Some(job) = app
        .model_download_jobs
        .iter()
...
fn render_model_download_card(
    ...
    theme: &Theme,
    glyphs: &Glyphs,
    card: (usize, &scala_model_library::ModelDownloadJob, Rect),
    ui_layout: &UiLayout,
) {
...
pub(crate) fn download_identity(job: &scala_model_library::ModelDownloadJob) -> String {
...
fn download_job_controls(job: &scala_model_library::ModelDownloadJob) -> Vec<DownloadJobAction> {
...
fn download_progress_bar(
    job: &scala_model_library::ModelDownloadJob,
    width: u16,
    animation_frame: u32,
    theme: &Theme,
    glyphs: &Glyphs,
...
fn download_phase_style(
    phase: scala_model_library::ModelOperationPhase,
    theme: &Theme,
) -> ratatui::style::Style {
    use scala_model_library::ModelOperationPhase;
    match phase {
...
fn render_logs(frame: &mut Frame<'_>, area: Rect, app: &App, theme: &Theme, ui_layout: &UiLayout) {
    ...
    let lines = app.logs[start..end].iter().map(|entry| {
        let (label, style) = match entry.level {
            scala_core::LogLevel::Info => ("INFO", theme.accent),
            scala_core::LogLevel::Warning => ("WARN", theme.warning),
            scala_core::LogLevel::Error => ("ERR ", theme.error),
        };
        Line::from(vec![
...
fn runtime_summary(app: &App, id: &scala_core::RuntimeId) -> String {
    app.runtime_list
        .as_ref()
        .and_then(|list| {
            list.installed
...
pub(super) fn render_settings_input(
    ...
    if let Some(editor) = &input.editor {
        ...
        let guidance = match editor.definition.kind {
            scala_core::SettingKind::OneWayFlag => {
                "Inherit removes this flag; the parent might still enable it."
            }
            scala_core::SettingKind::Path => {
        ...
        let constraints = if editor.definition.kind == scala_core::SettingKind::OneWayFlag {
    ...
    let field = if let Some(editor) = &input.editor {
        if editor.custom() {
            if editor.definition.kind == scala_core::SettingKind::StringList && !editor.editing_item
            {
            ...
            } else if editor.definition.kind == scala_core::SettingKind::JsonObject {
...
pub(crate) fn inspection_text(app: &App) -> Option<String> {
    let mut text = inspection_body(app).unwrap_or_else(|| match app.screen {
        ...
        Screen::Models => {
            if let scala_core::RegistryState::Failed { message } = &app.snapshot.registry_state {
                message.clone()
            } else {
...
fn server_text(app: &App) -> String {
    ...
    if let scala_core::ServerState::Unknown { message }
    | scala_core::ServerState::Failed { message } = &app.snapshot.server
    {
