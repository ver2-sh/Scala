...
impl App {
    ...
    pub fn handle_key(&mut self, key: KeyEvent, layout: &UiLayout) -> Update {
            ...
            && self.detail_text.is_none()
        {
            match key.code {
                KeyCode::Char('e') => {
                    self.pending_link_config = Some(scala_core::LinkConfig { enabled: true });
                ...
                MouseEventKind::ScrollUp | MouseEventKind::ScrollDown => {
                    if layout.hit_test(position) == Some(HoverTarget::SettingsInputField)
                        && self
                            ...
                            .is_some_and(|e| {
                                e.custom()
                                    && e.definition.kind == scala_core::SettingKind::JsonObject
                            })
                    {
    ...
    pub fn handle_paste(&mut self, text: &str) -> Update {
        ...
        if let Some(input) = &mut self.settings_input {
            ...
            if let Some(editor) = &input.editor {
                if !editor.custom()
                    || (editor.definition.kind == scala_core::SettingKind::StringList
                        && !editor.editing_item)
                {
                    return Update::None;
                }
    ...
    pub fn replace_model_download_jobs(&mut self, jobs: Vec<ModelDownloadJob>) {
        for job in jobs.iter().filter(|job| job.is_terminal()) {
            ...
            if !was_terminal {
                let level = if job.phase == scala_model_library::ModelOperationPhase::Failed {
                    LogLevel::Error
                } else {
                    LogLevel::Info
                };
    ...
    pub fn max_parallel_model_downloads(&self) -> usize {
        self.settings_state.as_ref().map_or(
            scala_model_library::DEFAULT_MAX_PARALLEL_DOWNLOADS,
            scala_model_library::max_parallel_downloads_from_settings,
        )
    }
                ...
                .and_then(|resolved| resolved.effective.get(id))
            {
                return SettingValueDisplay {
                    ...
                    source: match &setting.source {
                        scala_core::SettingSource::SettingsOverride => {
                            "Settings override".to_owned()
                        }
                        scala_core::SettingSource::RuntimeDefault => "runtime default".to_owned(),
                        scala_core::SettingSource::ModelProfile { .. } => {
                            "model profile".to_owned()
                        }
                        scala_core::SettingSource::Invocation => "boot inference".to_owned(),
                    },
                    state: match setting.source {
                        scala_core::SettingSource::SettingsOverride
                        | scala_core::SettingSource::RuntimeDefault => {
                            SettingPresentationState::Default
                        }
                        scala_core::SettingSource::ModelProfile { .. }
                        | scala_core::SettingSource::Invocation => {
                ...
                .map(|setting| {
                    format!(
                        ...
                        match setting.source {
                            ...
                            scala_core::SettingSource::Invocation => "invocation",
                        }
                    )
                })
    ...
    fn settings_running_effective_setting(
        &self,
        id: &SettingId,
    ) -> Option<&scala_core::EffectiveSetting> {
        if self.screen != Screen::ModelProfiles {
            return None;
        }
            ...
            KeyCode::Char('b') => {
                if let Some(profile) = self.selected_model_profile_value() {
                    self.benchmarks.pending =
                        Some(scala_engine::benchmark::BenchmarkRequest::Start {
    ...
    fn handle_typed_editor_key(&mut self, key: KeyEvent) -> Option<Update> {
        ...
        if editor.custom()
            && editor.definition.kind == scala_core::SettingKind::JsonObject
        ...
        let text = match local.as_ref() {
            Some(
                SettingValue::UnsignedIntegerOrChoice(
                    scala_core::UnsignedIntegerOrChoiceValue::Choice(_),
                )
                | SettingValue::GpuOffload(
                    scala_core::GpuOffload::None
                    | scala_core::GpuOffload::Auto
                    | scala_core::GpuOffload::All,
                ),
            ) => String::new(),
    ...
    fn request_load(&mut self) -> Update {
        if self.selected_remote_profile().is_some() {
            return self.request_remote_control(scala_engine::link::LinkAction::Load);
        }
        ...
        let Some(control) = &self.control else {
            self.notice = Some(if self.control_observation_pending() {
                ...
                "No running Scala control instance is available".to_owned()
            });
            return Update::Render;
        };
        if control
    ...
    fn request_unload(&mut self) -> Update {
        ...
        if self.selected_remote_profile().is_some() {
            return self.request_remote_control(scala_engine::link::LinkAction::Unload);
        }
        if self.control_busy {
            self.notice = Some("A control operation is already in progress".to_owned());
            return Update::Render;
    ...
    fn request_overview_unload(&mut self) -> Update {
        ...
        {
            let request = scala_engine::link::LinkControlRequest {
                node_id: node_id.clone(),
                profile_id: backend.model_profile_id.clone(),
                action: scala_engine::link::LinkAction::Unload,
            };
