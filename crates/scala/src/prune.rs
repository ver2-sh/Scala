...
pub async fn run(paths: &AppPaths, args: &crate::cli::PruneArgs, json: bool) -> Result<()> {
    ...
    let loaded = scala_core::LoadedConfig::load(paths)?;
    let registry = if args.all {
        scala_engine::EngineRegistry::default()
    } else {
    ...
    scala_engine::plan_runtime_prune(paths, &registry, &mut plan).await?;
    scala_model_library::plan_model_prune(paths, &downloads_root, &mut plan)?;
