...
use scala_engine::{ControlClient, ControlStatus, InstallationState, ModelServingCapabilities};
use serde_json::json;
...
pub async fn status(core: Arc<ApplicationCore>, json_output: bool) -> Result<()> {
    ...
    if let Some(message) = observation_error {
        snapshot.server = scala_core::ServerState::Unknown { message };
    }
    ...
    if json_output {
        ...
        if let scala_core::ServerState::Unknown { message } = &snapshot.server {
...
pub fn model_info(capabilities: &ModelServingCapabilities, json_output: bool) -> Result<()> {
    if json_output {
        ...
        if let Some(scala_core::ArtifactNativeIdentity::Ninfer(identity)) =
...
pub fn models_search(search: &scala_model_library::CatalogSearch, json_output: bool) -> Result<()> {
    if json_output {
...
pub fn model_operation(
    operation: &str,
    artifact: &scala_core::ModelArtifact,
    json_output: bool,
) -> Result<()> {
    if json_output {
        println!(
...
pub fn model_removed(model_id: &scala_core::ModelId, json_output: bool) -> Result<()> {
    if json_output {
        println!(
            "{}",
...
pub fn auth_status(status: &PublicAuthStatus, json_output: bool) -> Result<()> {
    if json_output {
        ...
        println!("Scala public API authentication");
        ...
        if status.insecure_remote {
            eprintln!(
                "WARNING: remote HTTP serving has authentication disabled; prompts, outputs, and credentials have no Scala transport protection."
            );
        }
    }
    Ok(())
...
pub fn api_keys(keys: &[ApiKeySummary], json_output: bool) -> Result<()> {
    if json_output {
        ...
        println!("No Scala API keys exist.");
    } else {
...
pub fn api_key_created(created: &CreatedApiKey, json_output: bool) -> Result<()> {
    ...
    if json_output {
        ...
        println!("Created Scala API key {} ({})", key.key_id, key.name);
...
pub fn api_key_revoked(key: &ApiKeySummary, json_output: bool) -> Result<()> {
    if json_output {
        ...
        println!("Revoked Scala API key {} ({}).", key.key_id, key.name);
    }
    Ok(())
}
...
pub fn control_operation(operation: &str, status: &ControlStatus, json_output: bool) -> Result<()> {
    if json_output {
                ...
                && let Some(provenance) = &backend.provenance
            {
                ...
                for (id, setting) in &provenance.settings.effective {
                    let source = match &setting.source {
                        scala_core::SettingSource::SettingsOverride => "Settings override",
                        scala_core::SettingSource::RuntimeDefault => "runtime default",
                        scala_core::SettingSource::ModelProfile { .. } => "model profile",
        ...
        for event in status.recent_events.iter().filter(|event| {
            matches!(
                event.level,
                scala_engine::RuntimeNoticeLevel::Warning | scala_engine::RuntimeNoticeLevel::Error
            )
        }) {
...
fn accelerator_binding_label(binding: &scala_core::AcceleratorBinding) -> String {
    binding
        .devices
        .iter()
        .enumerate()
...
pub fn runtimes_list(
    snapshot: &scala_engine::RuntimeListSnapshot,
    json_output: bool,
) -> Result<()> {
    if json_output {
...
pub fn runtimes_search(
    snapshot: &scala_engine::RuntimeSearchSnapshot,
    json_output: bool,
) -> Result<()> {
    if json_output {
        println!("{}", serde_json::to_string_pretty(snapshot)?);
...
pub fn runtime_operation(
    operation: &str,
    runtime: &scala_core::InstalledRuntime,
    json_output: bool,
) -> Result<()> {
    if json_output {
        println!(
...
pub fn runtime_removed(runtime_id: &scala_core::RuntimeId, json_output: bool) -> Result<()> {
    if json_output {
        println!(
            "{}",
            serde_json::to_string_pretty(&json!({
...
pub fn runtime_selections(
    operation: &str,
    selections: &scala_core::RuntimeSelections,
    json_output: bool,
) -> Result<()> {
    if json_output {
        println!(
...
pub fn runtime_updates(
    checks: &[scala_engine::RuntimeUpdateCheck],
    json_output: bool,
) -> Result<()> {
    if json_output {
        println!(
...
pub fn model_profiles(
    operation: &str,
    state: &scala_core::ModelProfilesState,
    selected: Option<&scala_core::ModelProfileId>,
    models: &[scala_core::ModelArtifact],
    json_output: bool,
) -> Result<()> {
    if let Some(id) = selected {
        ...
        let resolved = scala_core::ResolvedSettings::default();
...
pub fn model_profile(
    operation: &str,
    profile: &scala_core::ModelProfile,
    model: Option<&scala_core::ModelArtifact>,
    resolved: &scala_core::ResolvedSettings,
    json_output: bool,
) -> Result<()> {
    if json_output {
        println!(
...
pub fn model_profile_compatibility(
    ...
    selection: &scala_core::RuntimeSelection,
    schema: &scala_core::SettingsSchema,
    resolved: &scala_core::ResolvedSettings,
    compatibility: &scala_core::RuntimeCompatibility,
    json_output: bool,
) -> Result<()> {
    if json_output {
        println!(
...
pub fn settings_defaults(
    ...
    definitions: &[scala_core::SettingDefinition],
    runtime_id: Option<&scala_core::RuntimeId>,
    json_output: bool,
) -> Result<()> {
