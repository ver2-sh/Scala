    ...
    let registry = match composition::engine_registry_from_config(
        ...
        Err(error) => {
            checks.push(finding(
                ...
                Some(error.to_string()),
                &["Review [engine] configuration and reinstall Scala if built-in adapters are missing."],
            ));
            None
        }
    };
    ...
    let definitions = match registry.setting_definitions() {
        ...
        Err(error) => {
            checks.push(finding(
                ...
                Some(error.to_string()),
                &["Review engine configuration and installed Scala components."],
            ));
            return;
        }
    };
    ...
    if errors.is_empty() {
        ...
        for error in errors {
            checks.push(finding(
                ...
                Some(error),
                &["Use `scala settings show` to review current Server Settings and runtime defaults."],
            ));
        }
    }
}
...
fn validate_patch(
    ...
    for (id, value) in patch.iter() {
        let Some(definition) = definitions.iter().find(|definition| {
            &definition.id == id
                && match (&definition.scope, engine_id) {
                    (scala_core::SettingScope::Server, None) => true,
                    (scala_core::SettingScope::Runtime { engine_id: owner }, Some(engine_id)) => {
                        owner == engine_id
                    }
                    _ => false,
                }
        ...
        if let Some(engine_id) = engine_id
            && (!matches!(definition.scope, scala_core::SettingScope::Runtime { .. })
    ...
    for path in &config.models.paths {
        match fs::metadata(path) {
            ...
            Err(error) => checks.push(finding(
                ...
                &["Restore read and traversal access, then run `scala models list`."],
            )),
        }
    }
...
fn inspect_runtimes(
                ...
                inspection
                    ...
                    .filter(|runtime| {
                        runtime.runtime.manifest.acquisition_method
                            != scala_core::RuntimeAcquisitionMethod::ExternalBinary
                    })
                    .count()
            ),
            None,
    ...
    for warning in &inspection.adapter_warnings {
        checks.push(finding(
            ...
            Some(bounded_detail(warning)),
            &["Review `scala engines list`; managed runtime packs remain separate."],
        ));
    }
    for runtime in &inspection.installed {
        let manifest = &runtime.runtime.manifest;
        if manifest.acquisition_method == scala_core::RuntimeAcquisitionMethod::ExternalBinary {
            continue;
        }
    ...
    for (format, runtime_id) in &selections.format_defaults {
            ...
            && !status.runtime.manifest.supported_formats.contains(format)
        {
            checks.push(finding(
                ...
                &[&format!(
                    "Run:\n  scala runtimes clear-selection --format {}",
                    format.as_str()
                )],
            ));
            continue;
    ...
    for (model_id, runtime_id) in &selections.model_overrides {
        if !model_ids.contains(model_id) {
            checks.push(finding(
                ...
                &[&format!(
                    "Run:\n  scala runtimes clear-selection --model {model_id}"
                )],
            ));
        }
        inspect_selection(
            ...
            runtime_id,
            installed.get(runtime_id).copied(),
            format!("scala runtimes clear-selection --model {model_id}"),
            checks,
        );
    }
}
...
fn inspect_selection(
    target: String,
    runtime_id: &scala_core::RuntimeId,
    status: Option<&scala_engine::InstalledRuntimeStatus>,
    ...
    for model in models {
        match packs.resolve_from_local_inspection(model, None, None, inspection) {
            Ok(selection) if !selection.notices.is_empty() => checks.push(finding(
                ...
                "runtime_selections",
                DoctorStatus::Warning,
                format!("Model `{}` uses fallback runtime selection.", model.id),
                Some(bounded_detail(selection.notices.join("; "))),
                &[&format!(
                    "Run:\n  scala runtimes list\n  scala runtimes clear-selection --model {}",
                    model.id
                )],
            )),
            Ok(_) | Err(_) => {}
    ...
    for profile in profiles.profiles.values() {
        let Some(model) = models.get(&profile.model_id).copied() else {
            checks.push(finding(
                ...
                None,
                &[&format!(
                    "Run:\n  scala models list\n  scala model-profiles set-model {} <MODEL_ID>",
                    profile.id
                )],
            ));
            continue;
        ...
        let Some(adapter) = registry.get(profile.engine_id.as_str()) else {
            checks.push(finding(
                ...
                None,
                &[&format!(
                    "Run:\n  scala model-profiles set-engine {} <ENGINE>",
                    profile.id
                )],
            ));
            continue;
        };
        if let scala_engine::CompatibilityDecision::Unsupported { reason } =
        ...
        let mut resolved = match settings.resolve(
            ...
            Err(error) => {
                checks.push(finding(
                    ...
                    Some(error.to_string()),
                    &[&format!("Run:\n  scala model-profiles show {}", profile.id)],
                ));
                continue;
            }
        };
...
async fn inspect_control_and_bind(
    ...
    match ControlClient::discover_read_only(paths).await {
        Ok(client) => match client.status().await {
            Ok(status) => {
                    ...
                    .find(|backend| backend.lifecycle == BackendLifecycle::Failed)
                {
                    ...
                    checks.push(pass(
                        "control.healthy",
                        "control",
                        "A running Scala passed authenticated private-control verification.",
                        Some(format!(
                            "{} managed backend(s), {} running.",
                            status.backends.len(),
                            status.running_backend_count
                        )),
                    ));
                }
            }
            Err(error) => {
                ...
                checks.push(finding(
                    ...
                    "control",
                    DoctorStatus::Fail,
                    "A Scala public endpoint was found, but authenticated private control could not be verified.",
                    Some(bounded_detail(error.to_string())),
        ...
        Err(ControlClientError::Unavailable) => checks.push(pass(
            "control.stopped",
            "control",
            "No Scala is running; this is a normal stopped state.",
            None,
        )),
        Err(error) => {
            ownership_uncertain = true;
    ...
    if verified_running {
        checks.push(pass(
            "network.bind",
            "network",
            format!("Configured address {address} is owned by the verified Scala."),
            None,
        ));
        ...
        match tokio::net::TcpListener::bind(address).await {
            ...
            Err(error) => checks.push(finding(
                ...
                "network",
                DoctorStatus::Fail,
                format!("Configured address {address} is unavailable to Scala."),
                Some(error.to_string()),
...
fn add_basic_host_facts(checks: &mut Vec<DoctorFinding>, host: &scala_core::HostCapabilities) {
    checks.push(pass(
        "host.platform",
        "host",
        format!(
...
fn add_accelerator_facts(checks: &mut Vec<DoctorFinding>, host: &scala_core::HostCapabilities) {
...
async fn inspect_source_toolchain(
    host: &scala_core::HostCapabilities,
    checks: &mut Vec<DoctorFinding>,
) {
...
fn directory_finding(code: &str, label: &str, path: &Path) -> DoctorFinding {
    match fs::metadata(path) {
        Ok(metadata) if !metadata.is_dir() => finding(
            ...
            Some(path.display().to_string()),
            &["Move the conflicting object and let a normal Scala operation create the directory."],
        ),
        Ok(_) => match directory_write_access(path) {
            ...
            Err(error) => finding(
                ...
                Some(format!("{}: {error}", path.display())),
                &["Restore directory traversal and write access for the account running Scala."],
            ),
        },
...
fn directory_write_probe(path: &Path) -> Result<(), String> {
    ...
    for _ in 0..16 {
        ...
        let probe_path = path.join(format!(
            ".scala-doctor-write-probe-{}-{timestamp}-{probe_id}",
            std::process::id()
        ));
