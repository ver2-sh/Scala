...
impl StreamCalls {
    pub(super) fn deltas(&mut self, value: &Value) -> Result<Vec<InferenceEvent>, EngineError> {
        ...
        // Scala requests one assistant turn; multiple choices cannot share indexes.
        if choices.len() > 1 {
        ...
        for choice in choices {
            ...
            for call in calls {
                ...
                // Scala's ToolCallDelta contract. Arguments alone are appended.
        ...
        let result = async {
            ...
            if tools {
                let body = json!({"messages":[
                    {"role":"user","content":"scala_probe_user"},
                    {"role":"assistant","content":"","tool_calls":[{"id":"scala_probe_id","type":"function","function":{"name":"scala_probe_function","arguments":"{\"value\":\"scala_probe_argument\"}"}}]},
                    {"role":"tool","tool_call_id":"scala_probe_id","content":"scala_probe_result"},
                    {"role":"user","content":"scala_probe_continue"}],
                    "tools":[{"type":"function","function":{"name":"scala_probe_function","description":"scala_probe_definition","parameters":{"type":"object","properties":{"value":{"type":"string"}},"required":["value"]}}}],
                ...
                if let Ok(response) = self.client.post(format!("{endpoint}/apply-template")).timeout(PROPS_TIMEOUT).json(&body).send().await
                    ...
                    rendered = ["scala_probe_user", "scala_probe_function", "scala_probe_definition", "scala_probe_argument", "scala_probe_result", "scala_probe_continue"].iter().all(|s| prompt.contains(s));
                }
            }
