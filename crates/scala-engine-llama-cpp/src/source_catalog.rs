...
impl RuntimeCatalogProvider for LlamaCppSourceRuntimeCatalogProvider {
    ...
    async fn verify_candidate(
        ...
        {
            return Err(provider_error(
                "selected source runtime contract differs from the canonical Scala build recipe",
            ));
        }
        Ok(Some(live))
    }
...
async fn resolve_release_commit(
    ...
    if !scala_core::is_full_git_sha(&commit.sha)
        ...
        || (scala_core::is_full_git_sha(&release.target_commitish)
...
fn managed_cuda_advisories(recipe: ManagedCudaRecipe) -> Vec<String> {
    ...
    vec![
        "Scala builds this runtime from the exact official llama.cpp source revision; it is not an upstream CUDA binary"
            .to_owned(),
...
mod tests {
    ...
    use scala_engine::compatibility_for;
