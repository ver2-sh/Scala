...
// final context/slot contract behind Scala's back. They are removed from the
...
// q27 parses MODEL and TOKENIZER positionally. Scala owns those arguments,
...
fn validate_q27_settings_prelaunch(
    ...
    if let Some(SettingValue::UnsignedIntegerOrChoice(
        scala_core::UnsignedIntegerOrChoiceValue::Choice(value),
...
fn setting_toggle(settings: &scala_core::ResolvedSettings, id: &str) -> Option<bool> {
...
fn setting_unsigned(settings: &scala_core::ResolvedSettings, id: &str) -> Option<u64> {
    match settings.value(id) {
...
fn setting_float(settings: &scala_core::ResolvedSettings, id: &str) -> Option<f64> {
    match settings.value(id) {
...
fn setting_choice<'a>(settings: &'a scala_core::ResolvedSettings, id: &str) -> Option<&'a str> {
    match settings.value(id) {
...
fn setting_unsigned_with_source<'a>(
    settings: &'a scala_core::ResolvedSettings,
    id: &str,
) -> Option<(u64, &'a scala_core::SettingSource)> {
...
fn setting_source_label(source: &scala_core::SettingSource) -> &'static str {
    match source {
...
fn q27_has_configured_execution(settings: &scala_core::ResolvedSettings) -> bool {
...
fn settings_require_selected_kv(settings: &scala_core::ResolvedSettings) -> bool {
        ...
        if let Some(config) = config {
                ...
                .find(|name| conflicts_with_managed_environment(name))
            {
                configuration_error = Some(format!(
                    "environment variable `{name}` conflicts with the Scala-managed q27 backend contract"
                ));
            }
            ...
            {
                configuration_error = Some(format!(
                    "native argument `{argument}` conflicts with the Scala-managed q27 backend contract"
                ));
            }
    ...
    fn configured_backend_request(
        ...
        if (request.generation_settings.reasoning_enabled.is_some()
            ...
            || matches!(
                ...
                Some(scala_engine::ReasoningEffort::None)
            ))
...
impl EngineAdapter for Q27Adapter {
    ...
    fn validate_generation_settings(
        ...
        if matches!(
            ...
            (Some(true), Some(scala_engine::ReasoningEffort::None))
                | (
                    Some(false),
                    Some(
...
fn q27_setting_definitions() -> Vec<SettingDefinition> {
    ...
    definitions.extend([
        ...
        q27_definition(
            ...
            Some("Scala default: runtime template"),
        ),
        q27_definition(
            "q27.prompt_delivery",
            "Prompt delivery",
            ...
            Some("Scala default: runtime chat"),
        ),
        q27_definition(
            "q27.template_path",
            "Template path",
...
mod tests {
    ...
    fn request_generation_values_override_configured_defaults() {
        ...
        let (_, body) = Q27Adapter::configured_backend_request(
            ...
            &inference_request(GenerationSettingsPatch {
                ...
                top_p: Some(0.7),
                reasoning_effort: Some(scala_engine::ReasoningEffort::Low),
                ..Default::default()
            }),
            false,
        )
    ...
    fn request_reasoning_effort_is_deliberately_canonicalized() {
        ...
        let (_, body) = Q27Adapter::configured_backend_request(
            &execution,
            &inference_request(GenerationSettingsPatch {
                reasoning_effort: Some(scala_engine::ReasoningEffort::None),
                ..Default::default()
            }),
            false,
        )
