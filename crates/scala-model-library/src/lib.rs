...
impl ModelLibrary {
    ...
    pub fn with_max_parallel_downloads(
        ...
        Self {
            ...
            staging: paths.data_dir.join("models").join(".scala-staging"),
...
pub fn setting_definition() -> scala_core::SettingDefinition {
    scala_core::SettingDefinition {
        id: scala_core::SettingId::new(MAX_PARALLEL_DOWNLOADS_SETTING_ID)
        ...
        scope: scala_core::SettingScope::Server,
        category: scala_core::SettingCategory::Downloads,
        supported: true,
        unsupported_reason: None,
        unit: Some("downloads".to_owned()),
        default_preview: Some(scala_core::SettingDefaultPreview::new(
            DEFAULT_MAX_PARALLEL_DOWNLOADS.to_string(),
            scala_core::SettingDefaultSource::Scala,
        )),
    }
}

pub fn max_parallel_downloads_from_settings(settings: &scala_core::SettingsState) -> usize {
    settings
        .server_settings
        .iter()
        .find_map(|(id, value)| (id.as_str() == MAX_PARALLEL_DOWNLOADS_SETTING_ID).then_some(value))
        .and_then(|value| match value {
            scala_core::SettingValue::UnsignedInteger(value) => usize::try_from(*value).ok(),
            _ => None,
        })
    ...
    pub fn new() -> Self {
        Self {
            client: Client::builder()
                .user_agent(concat!("scala/", env!("CARGO_PKG_VERSION")))
                .build()
...
async fn cross_filesystem_install(stage: &Path, destination: &Path) -> Result<()> {
    ...
    let temp = parent.join(format!(".scala-install-{}", uuid::Uuid::new_v4()));
