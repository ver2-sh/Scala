//! Scala federation over Wayfinder named services. Owner facts stay on the owner.
use crate::{
...
use scala_core::{ApplicationCore, ModelProfileId, ModelProfilesStore};
use scala_engine::{RuntimeManager, link::*};
...
pub(crate) fn execution_profile_id(
    alias: String,
) -> std::result::Result<ModelProfileId, scala_core::SettingsError> {
    EXECUTION_PROFILE
...
impl Link {
    ...
    pub(crate) async fn run(
        ...
        tasks.spawn(async move {
            let host = scala_engine::detect_host_capabilities().await;
    ...
    async fn refresh(&self, address: std::net::SocketAddr) -> Result<()> {
        ...
        let refresh = stream::iter(status.nodes.into_iter().filter(|n| !n.local).map(
            |node| async move {
                ...
                // A member without a responding Scala service is not a Scala peer.
                match result {
                    Ok(state) => {
                        let peer = LinkPeer {
                            node_id: node.id,
    ...
    async fn request(&self, target: &str, operation: Operation) -> Result<Response> {
        let permit = self
            ...
            .clone()
            .try_acquire_owned()
            .map_err(|_| "Scala Link request capacity reached")?;
        let source = self
            .snapshot
            .read()
            .await
        ...
        if head.version != LINK_VERSION {
            return Err("Scala Link protocol version mismatch".into());
        }
    ...
    async fn request_state(&self, target: &str, operation: Operation) -> Result<NodeInventory> {
        ...
        if state.profiles.iter().any(|p| {
            !profiles.insert(p.id.clone())
                || scala_core::EngineId::new(p.engine_id.clone()).is_err()
        }) {
    ...
    async fn serve_connection(&self, mut stream: TcpStream) -> Result<()> {
            ...
            _ => {
                return send_response(
                    ...
                    link_error(
                        ...
                        "Malformed or oversized Scala Link request",
        ...
        {
            return send_response(&mut stream, link_error(StatusCode::BAD_REQUEST, "Invalid Scala Link version, identity, or hop count; forwarding loops are rejected", "invalid_link_route").into_response()).await;
        }
...
pub(crate) async fn route(
    ...
    if request
        .headers()
        .keys()
        .any(|name| name.as_str().starts_with("x-scala-link-"))
    {
        return link_error(
            StatusCode::BAD_REQUEST,
