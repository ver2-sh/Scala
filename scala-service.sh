...
SERVER_DIR="${SCALA_DIR:-$ROOT}"
UNIT_NAME="scala.service"
UNIT_PATH="/etc/systemd/system/${UNIT_NAME}"
GLOBAL_LINK="/usr/local/bin/scala"
BINARY="${SERVER_DIR}/target/release/scala"
...
usage() {
  cat <<EOF
Usage: ./scala-service.sh <command>

Manage Scala as a systemd service from a source checkout.
Also callable as ./scala-service.sh <command>.

Commands:
  install   Build Scala, install the systemd unit, install the
            global 'scala' application command, and start the service
...
Environment:
  SCALA_DIR  Scala checkout (default: directory containing this script)
EOF
}

require_server() {
  [[ -d "$SERVER_DIR" && -f "$SERVER_DIR/Cargo.toml" ]] || {
    echo "Scala not found at ${SERVER_DIR}." >&2
    echo "Run repos-setup.sh to clone it, or set SCALA_DIR." >&2
    exit 1
  }
}
...
build_server() {
  require_server
  require_cargo
  echo "Building Scala (release)..."
  (cd "$SERVER_DIR" && cargo build --release -p scala)
...
update_server() {
  ...
  if [[ -n "$dirty" ]]; then
    ...
    echo "Commit or stash your changes, then rerun './scala-service.sh update'." >&2
    exit 1
  fi
  ...
  if git -C "$SERVER_DIR" merge-base --is-ancestor HEAD "$upstream"; then
    ...
    echo "Rebase or merge manually, then rerun './scala-service.sh update'." >&2
    exit 1
  fi
  ...
  echo
  install_unit --built
  echo "Scala updated and restarted."
}
...
install_unit() {
  ...
  cat > "$tmp_unit" <<EOF
[Unit]
Description=Scala (local inference control plane and OpenAI-compatible gateway)
...
EOF
  ...
  # Link the application binary; the service helper must not shadow the CLI.
  if [[ -e "$GLOBAL_LINK" && ! -L "$GLOBAL_LINK" ]]; then
    echo "WARNING: ${GLOBAL_LINK} exists but is not a symlink; leaving it unchanged." >&2
  else
    local tmp_link="${ROOT}/.scala.new.$$"
    rm -f "$tmp_link"
    ln -s "$BINARY" "$tmp_link"
...
case "$cmd" in
  ...
  remove)
    ...
    "${SUDO[@]}" rm -f "$UNIT_PATH"
    "${SUDO[@]}" systemctl daemon-reload
    # Remove only the symlink owned by this source installation.
    ...
    echo "Removed ${UNIT_PATH}. Scala source was left untouched."
    ;;
  -h|--help|"")
    usage
    exit 0
