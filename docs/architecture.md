...
Acquisition requests explicitly carry stable/latest/exact intent plus installation, build, platform, architecture, and runtime-variant context. Exact acquisition uses a selector that structurally requires a validated non-empty version, source revision, or both; surrounding whitespace is normalized and empty values are rejected by both Rust construction and Serde. Installation provenance can retain source, acquisition method, exact revision, binary path and SHA-256, build options/toolchain, platform, architecture, runtime variant, and timestamp. Runtime provenance links model identity and optional hash to an exact engine revision, resolved settings, redacted native inputs, process identity, and launch time; unavailable facts remain optional.
...
`norted-api` owns the public HTTP protocol and server lifecycle. It implements only `GET /health` and `GET /v1/models`. Model listing uses the OpenAI-style list envelope and emits the four required core Model fields `id`, `object`, `created`, and `owned_by`; optional OpenAI fields without local semantics are omitted. `created` is the artifact's last-modified Unix timestamp. The server completes initial discovery before binding, so model listing receives a ready registry. This is limited Models-list compatibility, not full OpenAI compatibility. The Responses API remains planned primary inference work and is not present.
