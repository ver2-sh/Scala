...
protocols, without WSL. Managed runtime compatibility remains owner-local and
per-provider: Windows llama.cpp support does not imply Windows managed q27
support, and NInfer's managed Windows route is its own separately reviewed
portable provider rather than canonical upstream.
...
- `scala-engine-ninfer` owns the canonical source-snapshot catalog, the reviewed portable Windows release catalog, exact target-registry enumeration, `.ninfer` compatibility, source/external runtime probes, settings and GPU policy, startup-log observation, and private Chat JSON/SSE translation.
...
Canonical `Neroued/ninfer` has no packaged binary or install target. Its provider queries the canonical repository, resolves the current default branch and its full HEAD commit/tree metadata, and returns one `Latest` `SourceBuild` candidate. It does not synthesize Stable, semantic versions, release assets, or archive hashes. The human version is an explicit `git-<date>-<short-sha>` snapshot label; the full commit in identity/provenance is authoritative. A bounded cache of previously discovered source descriptors keeps a selected immutable commit addressable after HEAD advances, while installation still revalidates that exact commit/tree against the live provider.

A separate `natpate/ninfer-windows` provider covers the portable native Windows releases. It retains that repository's real provenance rather than claiming canonical upstream, admits only the reviewed v2-compatible `v0.7.x` generation — upstream `v0.8.0` onward moved to NInfer v3 artifacts that Scala does not load — and binds each candidate to exactly one `ninfer-windows-<version>-win64-cuda<toolkit>.zip` asset with an uploaded state, nonzero size, and a GitHub SHA-256 digest. `v0.7.1` is the reviewed candidate credited with the same capability domains as the reviewed source contract; other admitted `v0.7.x` releases keep truthful identity but remain needs-attention until explicitly reviewed. The release-asset arm preserves the archive's internal layout so `ninfer-serve.exe` keeps its required adjacent DLLs, probes the executable with the same `ninfer-serve --help` contract, and writes an `OfficialReleaseAsset` manifest with release tag, asset ID/name, verified archive and entrypoint digests. Windows x86_64 + CUDA + RTX 5090/sm_120a is the only platform contract; no WSL is involved. Managed Windows installs form their own `managed-windows-x86_64-cuda-sm120a` functional update line, so Linux source builds and Windows portable releases can never update toward each other.
...
ZIP and tar.gz extraction runs off the async executor and enforces entry-count and expanded-size limits. ZIP entry separators are normalized (`\` to `/`) before the containment checks because some Windows-produced archives record them that way; the checks then reject absolute paths, parent components, drive prefixes, colon tricks, duplicate entries, special devices/FIFOs, and overwrites. Tar symlink/hardlink targets are normalized and must resolve within staging; links are created only after their targets exist and are canonically contained.
