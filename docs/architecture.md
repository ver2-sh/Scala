...
The current source identity is `managed-portable-v3`; `managed-portable-v1` and `managed-portable-v2` remain immutable historical identities. V3 preserves V2's CMake/Ninja Release build, shared-library layout, and `CMAKE_BUILD_RPATH_USE_ORIGIN=ON` policy, so the executable and libraries kept together under `build/bin` use `$ORIGIN`-relative build-tree paths after staging moves into the immutable runtime store. It builds only `llama-server` at `build/bin/llama-server`. Its deterministic real-code CUDA policy remains `75-real;80-real;86-real;89-real;90-real;120a-real`, represented as exact compute capabilities 7.5, 8.0, 8.6, 8.9, 9.0, and 12.0 and accelerator target `sm_75+sm_80+sm_86+sm_89+sm_90+sm_120a`.

V3 explicitly admits CUDA Toolkit `>=12.8,<13.0`: 12.8 is the first Toolkit that can compile the fixed Blackwell `sm_120a` target, while the exclusive 13.0 ceiling prevents a later CUDA major family from silently changing the runtime contract. It requires NVIDIA Linux driver `525.60.13` or newer, the documented CUDA 12.x minor-version compatibility floor; NVIDIA documents CUDA 13.x separately at driver 580 or newer. The fixed policy contains real code rather than PTX, and every target is passed explicitly to nvcc as required by NVIDIA's minor-compatibility guidance. See NVIDIA's [CUDA minor-version compatibility table](https://docs.nvidia.com/deploy/cuda-compatibility/minor-version-compatibility.html) and [exact CUDA 12.x floor](https://docs.nvidia.com/deploy/cuda-compatibility/forward-compatibility.html). Before checkout or compilation, the generic prerequisite probe checks the Toolkit floor and exclusive ceiling plus the observed compiler's `--list-gpu-code`/`--list-gpu-arch` output for every explicit real/virtual CMake target. Exact CMake arguments plus the observed CMake/Ninja/C++/nvcc toolchain remain in build provenance. A known driver below the floor is incompatible; missing driver evidence remains needs-attention. Norted never installs driver, Toolkit, or toolchain packages.

Update discovery asks each engine for a logical functional variant and an optional explicit source-recipe generation. llama.cpp maps only the exact managed Linux x86_64 CUDA V1, V2, and V3 identities to one functional family ordered 1, 2, and 3. This lets a later recipe update an earlier immutable install, including when the source commit is identical, while rejecting recipe downgrades and leaving CPU, Vulkan, Windows CUDA, unknown future generations, and unrelated toolchain families on their exact variant lines. Pinned runtimes continue to report an available generation without applying it automatically. The generic source-prerequisite memoization key is derived beside the checker from the complete immutable recipe and prerequisite structs, so explicit CUDA target policies cannot share a compatibility result accidentally.
