...
- `norted-core` owns platform paths, schema-version-1 configuration, public-auth policy, the versioned atomic API-key store, the typed Serve Profile/load-setting domain and resolver, the versioned atomic profile store, model/auxiliary-artifact discovery, stable IDs, runtime identity/manifest/preferences data, host-independent provenance, application state, and process descriptors.
...
## Serve Profiles and load-setting resolution

`Engine`, `Runtime`, `ServeProfile`, artifact provenance, and request-time generation settings are distinct domains. A Serve Profile has typed identity/source, applicability, prompt/template delivery, generation defaults and override permissions, load defaults/minimums, engine-specific strategy, and capability requirements. Builder sidecars are read-only and package-relative; old package runtime policies deterministically synthesize the same immutable profile in memory. New packages bind profile path, size, SHA-256, ID, schema, and version. Explicit and retained legacy representations must agree, so there is only one execution recipe.

`LoadSettingDefinition` describes a stable common or engine-namespaced ID, typed value kind, constraints/choices, documentation, and exact-runtime support. `LoadProfilesState` schema 2 lives at `<data>/load-profiles.json`; it reads schema 1 and promotes each existing named Load Profile into a mutable load-only Serve Profile while preserving defaults and assignments. State distinguishes local assignments, Builder assignments, explicit None/raw selection, and inherited recommendation. It uses a dedicated cross-process lock plus atomic replacement and is never written into TOML or a runtime manifest.

One core resolver applies global, selected-engine, model, selected-profile load defaults, then invocation patches. It filters unrelated engine namespaces while retaining them in model/profile state, records the winning source for every value, and produces no entry for an upstream default. Invocation `--profile none` disables the recommendation; another invocation profile replaces the persisted selection for that load. Invocation values are never written back. Profile minimums and locked fields are constraints and reject a conflicting invocation rather than silently winning or losing precedence. Generation resolution is runtime defaults, profile defaults, then a request override only when the profile permits that field. As the final engine-neutral resolution step, relative structured `Path` values are lexically resolved beneath the absolute application data directory and parent traversal outside that base is rejected; absolute values remain valid. No filesystem canonicalization is required, so an upstream-created target may be absent. Every downstream consumer therefore receives the same absolute effective path.

Runtime selection evaluates the artifact facts, exact runtime evidence, and selected Serve Profile. A runtime may therefore support raw q27 while failing Dirk Quality. The adapter observes the exact executable interface, evaluates typed profile requirements, returns a profile-aware `LoadSettingsSchema`, validates resolved values, detects native argument/environment collisions, and translates only explicit values into arguments/removals. llama.cpp help observations are keyed by runtime ID and entrypoint hash. q27 keeps its immutable exact-source fingerprints and bounded startup proof for context/KV/W_MAX. NInfer keeps its exact source/native-container and schema-18 startup evidence. Neither receives capability credit from filenames or vague version assumptions. `LaunchRequest` carries the prepared model, exact runtime, selected Serve Profile, selected accelerator, private address, resolved settings, and exact schema. Adapters execute the supplied profile; they do not discover package policy independently.

`RuntimeProvenance.serve_profile` records ID/display name, source, schema/version, deterministic content hash, recommendation relationship/status, and effective external-template identity/hash. Artifact Builder lineage remains in the separate model/package identity even when another profile or None is selected. The separate `load_settings.effective` snapshot stores stable typed values and precedence sources. Because adapters translate and return this same resolved snapshot in `LaunchSpec`, a structured path shown in effective inspection is byte-for-byte the path passed to the runtime and retained in provenance. Existing normalized adapter/generation facts and redacted native arguments keep their prior meanings.
...
Private backend status carries model ID, engine ID, runtime ID/version/variant, executable SHA-256, PID, and private endpoint. `RuntimeProvenance` retains the exact immutable runtime manifest, selection source, selected Serve Profile identity/source/hash/recommendation status/template evidence, selected accelerator identity and observations, primary model facts including typed native identity, every materially used auxiliary artifact's role/canonical path/size/SHA-256, sanitized native arguments with option/value association and value hashes, every effective explicit/inherited environment variable name with a value hash, inheritance policy, authoritative effective sampler settings, process identity, endpoint, and launch time. For NInfer, the embedded manifest supplies source repository/commit/tree, recipe/toolchain, build/result hashes separately from the artifact's model/weights identity. Missing facts stay optional; applying a Builder profile to third-party weights does not change artifact lineage, and external acquisition never gains invented release or source-build provenance.

## Public protocol

Responses remains canonical and Chat Completions is compatibility-only. Both public parsers normalize `developer`, `system`, `user`, and `assistant` text into the same engine-neutral `InferenceMessage` list and attach a typed `GenerationSettingsPatch`. Chat `reasoning_effort` and Responses `reasoning.effort` share the same typed low/medium/high value. `RuntimeManager` asks the selected adapter and Serve Profile to validate explicit settings, rejects forbidden conflicts, and permits reasoning effort only for a selected q27 external-template/raw-completions profile, where it becomes a Sharp render input. It merges allowed sampler values with profile and observed runtime defaults for truthful public reporting. Runtime selection and immutable launch provenance do not mutate from request-time values.
