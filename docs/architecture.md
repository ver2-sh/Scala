...
A separate provider supplies Linux x86_64 CUDA as a managed source build, not as a fabricated release asset. It selects the newest valid official nightly release, resolves its tag through the canonical repository to full commit/tree/timestamp metadata, and produces an identity containing that exact tag and commit. The fixed recipe uses CMake/Ninja Release mode, `GGML_CUDA=ON`, `GGML_NATIVE=OFF`, and the upstream `llama-server` target/`build/bin/llama-server` entrypoint while disabling unrelated apps, tests, UI downloads, and optional fetched dependencies. The source candidate and recipe are reconstructed from the live tag before installation; the generic installer independently verifies the repository, checkout commit/tree, prerequisites, result probe, and persisted provenance. A later nightly is an ordinary source-runtime update with a distinct immutable identity.
