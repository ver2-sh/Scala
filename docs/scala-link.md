...
Scala Link federates Scala instances over an existing Wayfinder network.
...
Wayfinder: identity / membership / encrypted application service transport
  ↓
Scala Link: models / GPUs / profiles / inference / remote load state
  ↓
Local Scala runtime: NInfer / llama.cpp / q27 / etc.
```

Wayfinder discovers generic nodes. Scala independently opens `scala.link.v1`
on reachable nodes and accepts compatible owner reports. Nodes without that
service are normal and remain absent from Scala's peer list. Wayfinder maintains
...
is not live free-memory telemetry or a scheduling decision. Scala also reports
...
3. Start Scala. Scala automatically discovers the generic local
   application endpoint and registers `scala.link.v1`. No address, credential,
   capability file or application setup in Wayfinder is needed.

The **Scala Link** screen shows connection state, full local node ID and peer
observations. Press `e` to enable or `x` to disable; `scala link status`
reports saved configuration and observed state. Saved enable/disable changes
apply when Scala starts. Wayfinder does not need a restart.
...
discovery. Scala never starts or administers Wayfinder automatically.

## Authority and boundaries

The local Model Registry, Model Profiles store, runtime manager and benchmark
store remain authoritative only for this machine. Scala queries owner reports
...
files, run a benchmark or execute a shell through the Scala Link protocol.
...
permissions authorize the application; Scala verifies endpoint permissions and
...
On native Windows 11, Scala connects to `\\.\pipe\wayfinder-app-v1`.
...
executables under the same Windows account. Scala verifies the pipe owner is
...
Scala keeps a registration session open and binds an ephemeral authenticated
...
persisted. Scala reconnects and re-registers after Wayfinder starts or restarts.
...
seconds. A node without a responding Scala service never becomes a Scala peer.
...
routes, unknown operations, arbitrary API paths and public `x-scala-link-*`
...
Scala admits at most 16 concurrent Link operations per node, including discovery;
...
These routes are on Scala's private control listener, never its public listener.
...
Disconnecting Wayfinder cancels streams without stopping local Scala serving.
