...
The separate llama.cpp managed-source provider covers Linux x86_64 CUDA because upstream does not publish a comparable Ubuntu CUDA archive. It admits an official nightly only when that exact full commit/tree proves the owned CMake contract, then builds only `llama-server`; this is never labeled as an upstream CUDA binary. The current immutable recipe generation is `managed-portable-v3`. It builds fixed real-code targets `sm_75`, `sm_80`, `sm_86`, `sm_89`, `sm_90`, and `sm_120a` with CUDA Toolkit `>=12.8,<13.0`, and records NVIDIA's CUDA 12.x Linux minor-compatibility driver floor, `525.60.13`. Shared libraries remain beside the server under origin-relative build RPATHs, so staging relocation does not embed the temporary source path. The release tag, exact source identity, recipe, observed toolchain, and executable digest remain in provenance; V1 and V2 identities remain immutable and V3 is an explicit update generation. Norted validates but never installs CUDA, the NVIDIA driver, or other toolchain packages.
