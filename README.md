...
Native arguments are appended exactly as configured. Norted rejects native `--model`/`-m`, `--host`, and `--port` arguments because it must own the model and private binding. It otherwise emits only those three managed settings, preserving llama.cpp's evolving defaults unless the user explicitly supplies a native override. The child inherits the serving process's environment and applies configured `[engine."llama.cpp".env]` entries over it. Provenance records that parent inheritance occurred, records configured environment names and value hashes rather than raw values, and does not snapshot inherited values. Native-argument provenance conservatively retains bare dash-prefixed arguments but redacts non-option values and the value in `--name=value` forms.

Each time the adapter is asked to probe, it canonicalizes the configured file, runs `llama-server --version` and `llama-server --help`, verifies that `--model`, `--host`, and `--port` are advertised, and hashes the binary with SHA-256. Probe results are not cached in the adapter, so manager initialization and later load checks revalidate the current binary. Version and revision are recorded only when the binary reports them. `norted-server engines list` shows the latest stored or locally obtained installed, disabled, or invalid probe state. The acquisition method is recorded truthfully as an external binary; Norted does not claim to have installed it.
...
`load` contacts the already-running `serve` process; it never launches a hidden second server. Norted resolves the exact discovered artifact, chooses the compatible installed engine, starts `llama-server`, and waits for its `/health` endpoint to report ready before returning `Running`. Startup is bounded to five minutes. If another model is already active, the request fails instead of replacing it.
...
Configuration, application data, runtime state, cache, and logs use separate operating-system application directories. The TUI starts discovery only after terminal initialization and displays `Scanning` until ready. `models list`, `status`, and `serve` await deterministic discovery; `engines list` and `config show` do not initiate a scan. `NO_COLOR` disables TUI colour independently of configuration, while `tui.unicode = false` selects intentional ASCII glyphs.
