...
Managed NInfer uses a distinct source-build transaction:
...
Its top-level pages are Overview, Models, Runtimes, Server, Logs, Settings, and Help. The Runtimes page shows exact format selections and installed packs, then opens an interactive available-runtime search with keyboard filtering, arrow or `j`/`k` movement, mouse hover/click, details, and install actions. Release downloads retain real byte progress. Source installs instead expose Checking prerequisites, Fetching source, Verifying source, Configuring, Building, Probing, Installed, or Failed without inventing byte totals. Installed NInfer details include short commit/tree, recipe, CMake, and CUDA provenance.
...
- Managed source builds currently exist only for the exact official NInfer Linux x86_64/RTX 5090/sm_120a contract; q27 source-only releases remain unmanaged.
