...
The separate llama.cpp managed-source provider covers Linux x86_64 CUDA because upstream does not publish a comparable Ubuntu CUDA archive. One admitted exact nightly commit/tree exposes two intentional parallel source variants, neither labeled as an upstream CUDA binary: immutable CUDA-12 `managed-portable-v3` requires Toolkit `>=12.8,<13.0` and Linux driver `>=525.60.13`, while `managed-portable-cuda13-v1` requires Toolkit `>=13.0,<14.0` and driver `>=580.65.06`. Both build only `llama-server` with fixed real-code targets `sm_75`, `sm_80`, `sm_86`, `sm_89`, `sm_90`, and `sm_120a`, origin-relative build RPATHs, and exact source/recipe/toolchain/executable provenance. Historical V1/V2 identities remain immutable; V1 is retained for update discovery but is not an automatic serving candidate because its pre-relocatability recipe can retain stale build-tree library paths. Norted validates but never installs CUDA, the NVIDIA driver, or other toolchain packages.
