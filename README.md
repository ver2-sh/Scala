...
The separate llama.cpp managed-source provider covers Linux x86_64 CUDA because upstream does not publish a comparable Ubuntu CUDA archive. It resolves the newest valid official nightly tag to an immutable full commit/tree and builds only `llama-server` with the Norted-owned Release, `GGML_CUDA=ON`, `GGML_NATIVE=OFF` portable recipe. This is never labeled as an upstream CUDA binary. The release tag, source timestamp, exact commit/tree, fixed recipe, observed CMake/Ninja/C++/nvcc toolchain, and built executable digest remain in provenance; installation revalidates the canonical tag, source identity, and recipe before building.
...
- The llama.cpp Linux CUDA build requires Git, CMake 3.18 or newer, Ninja, a C++17 compiler, and an installed CUDA Toolkit with `nvcc`. Upstream does not publish one universal minimum Toolkit version for the portable recipe; the Toolkit itself determines which CUDA GPU architectures the resulting build covers. Norted validates these tools but does not install Ubuntu packages or the NVIDIA driver/toolkit.
