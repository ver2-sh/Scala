...
A separate provider covers the portable native Windows NInfer packages published by `natpate/ninfer-windows`. That port is not canonical `Neroued/ninfer`, and Scala retains its real repository and release provenance in the runtime identity rather than relabeling it. The provider admits only the v2-compatible `v0.7.x` generation: upstream `v0.8.0` and later moved to NInfer v3 artifacts that Scala does not load, and earlier portable releases predate the reviewed contract, so neither is ever presented as an installable candidate. `v0.7.1` is the known reviewed candidate and receives the same capability domains as the reviewed source contract; any other admitted `v0.7.x` release keeps truthful identity but remains needs-attention until explicitly reviewed. Candidates carry the GitHub release asset ID, name, size, and SHA-256 digest; installation re-validates the live release metadata, verifies the exact archive bytes, extracts the package transactionally while retaining the required DLL layout beside `ninfer-serve.exe`, and runs the same adapter `--help` probe before activation. The package is native Windows x86_64 + CUDA + RTX 5090/sm_120a and does not use or require WSL. Its runtime update line is `managed-windows-x86_64-cuda-sm120a`, fully separate from the Linux source line, so neither side can update toward the other and a Windows install is never moved to a v3 release.
